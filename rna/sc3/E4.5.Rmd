...
io$outdir <- "/Users/ricard/NMT-seq_EB+ESC/rna/sc3/out"
...
opts$cells <- fread(io$sample_metadata) %>% 
  .[pass_rnaQC==T & culture=="EB_Day2" & phenotype%in%c("Tet_KO","Tet_WT"),id_rna]
```

<!-- Load sample metadata -->
```{r}
sample_metadata <- fread(io$sample_metadata) %>% 
  # .[,c("id_rna","culture","phenotype","lineage10x","lineage10x_2")] %>% 
...
<!-- Run sc3 -->
```{r, cache=T}
sce_sc3 <- sc3(sce, ks=c(2), gene_filter=TRUE, pct_dropout_min=20, pct_dropout_max=80, n_cores=1, biology=TRUE, rand_seed=1)
...
<!-- tSNE -->
```{r}
plotTSNE(sce_sc3, ncomponents=2, colour_by="phenotype", run_args=list("rand_seed"=1)) +
...
<!-- PCA -->
```{r}
plotPCA(sce_sc3, ncomponents=c(2,3), colour_by="lineage") +
...
<!-- Heatmap -->
```{r}
sc3_plot_expression(sce_sc3, k=2, show_pdata=c("lineage","phenotype"))
```
...
sc3_plot_markers(sce_sc3, k=2, show_pdata=c("phenotype"), auroc=0.7, p.val=0.1)
# dev.off()
```
