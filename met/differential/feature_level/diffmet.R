...
p$add_argument('-groupA', '--groupA',       type="character",  nargs='+',  help='group A')
p$add_argument('-groupB', '--groupB',       type="character",  nargs='+',  help='group B')
...
# Define groups
opts$groupA <- args$groupA
opts$groupB <- args$groupB

# Subset top most variable sites
opts$number_features <- NA

# Filter by coverage
opts$min.CpGs <- 1                # Minimum number of CpG per genomic feature in each cell
opts$min.cells <- args$min.cells  # Minimum number of cells per genomic feature in each group
...
## START TESTING ##
# opts$annos <- "prom_2000_2000"
# opts$groupA <- c("Epiblast_Day2_WT")
# opts$groupB <- c("Epiblast_Day2_KO")
# opts$min.cells <- 5
# opts$number_features <- NA
## END TESTING ##
...
opts$cells <- fread(io$sample.metadata) %>% 
  .[,group:=paste(lineage10x_2,day,genotype,sep="_")] %>%
  .[pass_metQC==T & group%in%c(opts$groupA,opts$groupB),id_met]
...
data <- data %>% merge(sample_metadata[,c("id_met","group")], by="id_met")

# Define the two exclusive groups
data[,group:=as.factor( c("A","B")[as.numeric(group%in%opts$groupB)+1] )]
sample_metadata[,group:=as.factor( c("A","B")[as.numeric(group%in%opts$groupB)+1] )]
