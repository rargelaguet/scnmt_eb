...
# Filter by coverage
opts$min.GpCs <- 5                # Minimum number of GpC per genomic feature in each cell
...
opts$cells <- fread(io$sample.metadata) %>% 
  ...
  .[pass_accQC==T & group%in%c(opts$groupA,opts$groupB),id_acc]
...
data <- data %>% merge(sample_metadata[,c("id_acc","group")], by="id_acc")
