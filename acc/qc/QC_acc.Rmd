...
io$in.accdir <- "/Users/ricard/data/NMT-seq_EB+ESC/acc/gpc_level"
...
opts$acc_mean_threshold.top <- 40
```
...
for (cell in opts$cells) {
  if (file.exists(sprintf("%s/%s.tsv.gz",io$in.accdir,cell))) {
    data <- fread(cmd=sprintf("zcat < %s/%s.tsv.gz",io$in.accdir,cell), sep="\t", verbose=F, showProgress=F)
