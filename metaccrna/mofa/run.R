...
if (grepl("ricard",Sys.info()['nodename'])) {
  ...
  source("/homes/ricard/NMT-seq_EB+ESC/metaccrna/mofa/load_settings.R")
  use_python("/nfs/software/stegle/users/ricard/conda-envs/py3/bin/python")
}


if (grepl("ricard",Sys.info()['nodename'])) {
  source("/Users/ricard/NMT-seq_EB+ESC/metaccrna/mofa/load_data.R")
} else {
  source("/homes/ricard/NMT-seq_EB+ESC/metaccrna/mofa/load_data.R")
}
...
# Train the model
outfile <- sprintf("%s/hdf5/test3.hdf5",io$outdir)
